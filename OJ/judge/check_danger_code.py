...
def check_danger_code(solution_id, language):
    ...
    if language in ['python2', 'python3']:
        code = codecs.open('/work/%s/main.py' % solution_id).readlines()
        support_modules = [
            're',  # 正则表达式
            'sys',  # sys.stdin
            'string',  # 字符串处理
    ...
    #        return True
    if language == 'go':
        code = codecs.open('/work/%s/main.go' % solution_id).read()
        danger_package = [
